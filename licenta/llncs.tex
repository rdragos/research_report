...
Presupunând că un utilizator Bob, avănd email-ul \textit{bob@mail.com} vrea să partajeze un fișier cu numele \textit{sample.txt} către $3$ instanțe iar reconstituirea lui să fie posibilă din oricare $2$ mașini. DealerApp parcurge următorii pași:
\begin{itemize}
	\item Rulează schema de distribuire definită în Figura \ref{fig:dealerapp_distribution};
	\item Se calculează $sgn_i \leftarrow \mathcal{H}(s_i || sk || sample.txt)$ $1 \leq i \leq 3$;
	\item Trimite unei instanțe $i$ perechea $(sgn_i, s_i)$ $1 \leq i \leq 3$;
	\item Instanța $i$ salvează într-o tabelă hash cu cheia $sgn_i$ valoarea $s_i$ (\textit{hset $sgn_i$ $s_i$}) $1 \leq i \leq 3$.
\end{itemize}

Pentru că funcția $\mathcal{H}$ este resistentă la coliziuni iar cheia $sk$ este întotdeauna privată lui Bob, unui atacator îi este dificil să modifice integritatea unei componente $sgn_i, s_i$ de pe un server Redis fără ca acesta să fie detectat.
