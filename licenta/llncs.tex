...
În acest moment dacă un adversar preia controlul serverului ce distribuie componentele (mai exact descoperă cheile secrete folosite pentru AWS - Amazon Web Services), fișierele partajate sunt compromise. Bineînțeles, arhitectura poate fi ușor extinsă la mai multe servere (nu doar unul singur) folosind structura GridSharing, evitând astfel atacul anterior \todo{poate?}.
...
O dată ce utilizatorul se autentifică, acesta are acces la o listă cu toate fișierele partajate de el. Pentru a distribui un fișier pe instanțele EC2, alege alege numărul de mașini la care să-l împartă și dimensiune pragului pentru care se poate face reconstituirea. La reconstituire are la dispoziție o listă cu toate fișierele partajate pe de la înregistrarea sa până în momentul actual.


\section{Distribuția unui fișier}

Fișierul postat de utilizatorul autentificat este preluat de un API și distribuit conform figurii \ref{fig:dealerapp_distribution}. Codul sursă pentru schema Shamir a fost preluat de pe GitHub (sub licența Apache) și modificat conform necesităților \cite{website:pysss-github}.
...
\hspace{.1in}
\textbf{Date de ieșire}: $n$ șiruri binare distribuite la noduri din rețea;
\medskip
...
\hspace{.1in}
\textbf{Inițializare}: Un șir binar $\mathcal{S} \leftarrow \emptyset$;
\medskip

\hspace{.1in}
\textbf{Reconstrucție}: Serverul DealerApp pentru fiecare $x_j, y_j$ din $s_1$:
	\begin{itemize}
		\item Pentru fiecare șir binar din cele $k$: $xp_i \leftarrow x_{i_j}, yp_i \leftarrow y_{i_j}$ $1 \leq i \leq k$, unde cu $x_{i_j}, y_{i_j}$ am notat octetul de pe poziția $j$ din șirul $i$ pentru $x$, respectiv $y$;
...
\begin{itemize}
 	...
 	\item Posibilitatea de a încerca gratis, timp de $1$ an, serviciile AWS.
\end{itemize}
...
AWS recomandă înlocuirea acestei perechi o dată la $90$ de zile pentru creșterea securității contului.
...
Regulile unui grup constă în blocarea sau admiterea anumitor porturi (sau intervale) corespunzătoare protocoalelor TCP sau UDP dar și un număr de adrese IP care pot accesa mașinile asociate grupului.
...
Bineînțeles, atunci cănd este necesară obținerea de log-uri de pe o anume instanță, se poate asocia incă un grup pentru efectua conexiunea prin SSH la acea mașină prin protocolul TCP având portul cu numărul $22$.
...
AMI sunt accesibile atât în consola AWS cât și prin intermediul API-urilor (Boto, AWS-CLI) pentru o manevrare mai ușoară a lor. 
