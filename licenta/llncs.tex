...
În acest moment dacă un adversar preia controlul serverului ce distribuie componentele (mai exact descoperă cheile secrete folosite pentru AWS), fișierele partajate sunt compromise. Bineînțeles, arhitectura poate fi ușor extinsă la mai multe servere (nu doar unul singur) folosind structura GridSharing, evitând astfel atacul anterior \todo{poate?}.
...
Fișierul postat de utilizatorul autentificat este preluat de un API și distribuit conform figurei \ref{fig:dealerapp_distribution}. Codul pentru schema Shamir a fost preluat de pe GitHub (sub licența Apache) și modificat conform necesităților \cite{website:pysss-github}.

De asemenea, modul în care erau generați coeficienții aleatori în \cite{website:pysss-github} a fost modificat (din $random.randint$ în $os.random$).
...
Studii recente arată că deși, practic, cele mai sigure generatoare de numere pseudo-aleatoare sunt $/dev/random$ și $/dev/urandom$ din sistemele UNIX, acestea nu sunt robuste \cite{dodis2013security}. După Principala problemă fiind extragerea entropiei precum și măsurarea ei.
