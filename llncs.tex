...
Schema Shamir nu ofer\u{a} siguran\c{t}\u{a} \^{i}n ceea ce prive\c{s}te detectarea sau actualizarea unor componente incorecte introduse de un atacator. Metoda cea mai des folosit\u{a} este determinarea validit\u{a}\c{t}ii componentelor prin utilizarea semn\u{a}turilor electronice.
...
\todo{de ce ai nevoie de subsectiune daca ai una singura?  }
...
\^{I}mpreun\u{a} cu mentorul am analizat sistemul Alouneh et al. prezentat \^{i}n Sec\c{t}iunea \ref{sec:desc_alouneh}  \todo{Sec\c{t}iunile mereu cu litera mare si numarul nu in paranteze! - am modificat eu} 
 \cite{AAMK:2013}.
...
\label{sec:results}
\subsection{\todo{Vulnerabilit\u{a}\c{t}i eviden\c{t}iate}}

Spre deosebire de schema Shamir, unde coeficien\c{t}ii (\todo{cu excep\c{t}ia termenului liber, care este egal cu secretul}) sunt ale\c{s}i \^{i}ntr-un mod aleator uniform, sistemul propus de Alouneh et al. folose\c{s}te o schem\u{a} modificat\u{a} Shamir \^{i}n care coeficien\c{t}ii sunt extra\c{s}i din con\c{t}inutul fi\c{s}ierelor originale.
Alegerea este motivat\u{a} de faptul ca mul\c{t}imea componentelor \c{s}i efortul computa\c{t}ional depus pentru generarea coeficien\c{t}ilor se reduce la un factor de $k$ \todo{nu se reduce la k, se reduce de k ori}, spre deosebire de schema Shamir.

\todo{explica simplu ca asta inseamna ca daca se partajeaza un acelasi fisier de mai multe ori se obtin intotdeauna aceleasi componente}
Determinismul duce la c\^{a}teva atacuri simple \^{i}n momentul \^{i}n care un atacator ob\c{t}ine informa\c{t}iile stocate \^{i}ntr-un nod, indiferent de m\u{a}rimea pragului folosit \^{i}n metoda de partajare. \^{I}n acest sens, am eviden\c{t}iat $2$ atacuri simple \^{i}n cazul \^{i}n care componentele sunt calculate in ordine ($f(01), f(02), \dots$ \todo{Repet, gandeste-te ca va citi raportul cineva care nu are nici o idee despre ce inseamna si trebuie sa inteleaga}):
\begin{itemize}
	\item Detectarea tipului unui fi\c{s}ier
	\item Detectarea tipului de con\c{t}inut unui fi\c{s}ier \todo{reformulare}
\end{itemize}
...
\todo{din nou nu dai detalii - de unde sa stie cititorul ca nu e precizat in articol cum trebuie facut padding? ce inseamna completarea fisierului s?}

\subsubsection{Detectarea tipului de fi\c{s}ier}\hspace*{\fill} \\
\label{subsec:file_type_detection}

\^{I}n sistemele de operare \todo{In sistemele de operare?!}, la inceputul fiec\u{a}rui fi\c{s}ier se afl\u{a} o secven\c{t}\u{a} de octe\c{t}i (denumit\u{a} semnatur\u{a} sau antet) \todo{cu rolul de a identifica} tipul acestuia. \^{I}n tabelul {\ref{table:sign}} g\u{a}sim $4$ \todo{4?! nu ai verificat ce ai scris!} din cele mai uzuale antete.
\todo{Tabelul x contine / indica ... - incearca sa nu mai scrii totul la persoana 1 - nu e formal; tine minte ca si la indicatiile pentru autori cand am scris articolul era mentionat; referirea la tablouri / figuri se face mereu cu majuscula: Fig.x Tabelul x}

Consider\^{a}nd cazul \^{i}n care dorim sa partaj\u{a}m un fi\c{s}ier \textit{pdf} cu ajutorul sistemului descris in \todo{S}ec\c{t}iunea \ref{sec:desc_alouneh}, folosind $k \leq 4$. Polinomul corespunz\u{a}tor $f(x)$ va fi intotdeauna acela\c{s}i. Presupun\^{a}nd ca numerotarea nodului $i$ este aceea\c{s}i, putem determina cu u\c{s}urin\c{t}\u{a} dac\u{a} este stocat un fi\c{s}ier \textit{pdf} f\u{a}r\u{a} a lua \^{i}n calcul con\c{t}inutul fi\c{s}ierului.
...
\todo{Care este diferenta fata de ce ai scris in paragraful anterior?}
...
\todo{aici idea e putin alta; daca in primul caz comparam 2 fisiere distincte ca sa identificam ce tip de informatie contine unul dintre ele stiind ce contine celalalt, aici avem multiple componente identice care indica faptul ca exista secvente repetitive in continutul fisierului; extinde putin}
...
\todo{Tabelul {\ref{table:k2}} prezint\u{a}} valoarea maxim\u{a} a nodului $i$ pentru care $f_l(i) \neq f_c(i)$. Valoarea $-1$ indic\u{a} lips\u{a} de coliziuni ale lui $f_c(x)$, $f_l(x)$ pentru $i$, $i = {1,2,\dots,255}$ ($\not\exists 1 \leq i \leq 255$ a.\^{i}. $f_l(i) = f_c(i)$)).
\todo {verifica la final ca ai peste tot Tabelul x, Fig.x, Sectiunea x - cu majuscula}
...
Pentru experiment am ales $3$ fi\c{s}iere \textit{pdf} disponibile online la {\cite{Europass:2014}}:
\begin{itemize}
...
Primele dou\u{a} fi\c{s}iere reprezint\u{a} \c{s}ablonul pentru un CV european \^{i}n limba bulgar\u{a}, respectiv danez\u{a}. Observ\u{a}m c\u{a} nu doar con\c{t}inutul \c{s}abloanelor difer\u{a}, dar \c{s}i limba \^{i}n care au fost traduse. Cel de-al treilea fi\c{s}ier este complet diferit de primele dou\u{a}, fiind un document personal \^{i}n limba rom\^{a}n\u{a}, folosit pentru organizarea timpului \todo{organizarea timpului? nu mobilitati?}.
...
\todo{reformulare! Pe baza rezultatelor ob\c{t}inute am redactat (\^{i}mpreun\u{a} cu mentorul) un articol, aflat momentan \^{i}n procesul de recenzie la Journal of Control Engineering and Applied Informatics (jurnal indexat ISI,categoria C). \cite{Ceai:2015}} 

\todo{Bibliografie:\\
1. Accesat ultima data - trebuie inlocuit cu Ultima accesare\\
2. diffie hellman cu majuscule - in fisierul bib pui in \{ \}ce vrei sa apara cu litere mari\\
3. ai inca la libliografie quantum - adauga info in text sau sterge de la bibliografie - nu cred ca e utilizata}
